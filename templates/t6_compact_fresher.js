...
window.ResumeTemplates.t6_compact_fresher = {
  ...
  render(data, accentColor) {
    ...
    const photoHTML = window.ResumeHelpers.photoContent(profile, initials, 'font-size:31px;font-weight:700;color:#fff');

    const sHead = (title) => `
      <div style="background:${accent};color:#fff;font-size:13px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;padding:3px 8px;margin:10px 0 6px;border-radius:2px">${title}</div>`;

    const workHTML = work.map(w => `
      <div style="margin-bottom:7px;display:flex;gap:8px">
        <div style="min-width:58px;text-align:right;font-size:11px;color:#999;padding-top:1px;line-height:1.4">${[w.from,w.to].filter(Boolean).join('<br>')}</div>
        <div style="width:1px;background:#e5e5e5;flex-shrink:0"></div>
        <div style="flex:1">
          <span style="font-weight:700;font-size:14px;color:#1a1a1a">${w.role||''}</span>
          ${w.company?`<span style="font-size:13px;color:${accent};margin-left:5px">@ ${w.company}</span>`:''}
          ${w.desc?`<div style="font-size:12px;color:#555;line-height:1.5;margin-top:2px">${w.desc.replace(/\n/g,'<br>')}</div>`:''}
        </div>
      </div>`).join('');

    const eduHTML = edu.map(e => `
      <div style="margin-bottom:6px;display:flex;gap:8px">
        <div style="min-width:58px;text-align:right;font-size:11px;color:#999;padding-top:1px;line-height:1.4">${[e.from,e.to].filter(Boolean).join('<br>')}</div>
        <div style="width:1px;background:#e5e5e5;flex-shrink:0"></div>
        <div style="flex:1">
          <span style="font-weight:700;font-size:14px;color:#1a1a1a">${e.degree||''}</span>
          <span style="font-size:13px;color:${accent};margin-left:5px">${e.school||''}</span>
          ${e.gpa?`<span style="font-size:11px;color:#999;margin-left:4px">· ${e.gpa}</span>`:''}
        </div>
      </div>`).join('');

    const skillsHTML = skills.length ? `
      <div style="display:flex;flex-wrap:wrap;gap:3px">
        ${skills.map(s=>`<span style="background:#f0fdf4;border:1px solid #bbf7d0;color:#15803d;padding:1px 7px;border-radius:10px;font-size:12px">${s.name}</span>`).join('')}
      </div>` : '';

    const customHTML = (customSections||[]).map(sec => `
      ${sHead(sec.title)}
      ${sec.items.map(it=>`<div style="margin-bottom:6px;padding-left:8px;border-left:2px solid #e5e5e5"><span style="font-weight:700;font-size:14px;color:#1a1a1a">${it.title||''}</span>${it.subtitle?`<span style="font-size:13px;color:${accent};margin-left:5px">${it.subtitle}</span>`:''}<span style="font-size:11px;color:#999;margin-left:5px">${it.date||''}</span>${it.desc?`<div style="font-size:12px;color:#555;line-height:1.5">${it.desc.replace(/\n/g,'<br>')}</div>`:''}</div>`).join('')}
    `).join('');
      ...
      <div style="width:210mm;min-height:297mm;font-family:inherit;background:#fff;padding:20px 22px;box-sizing:border-box">
        ...
        <div style="display:flex;align-items:center;gap:14px;padding-bottom:12px;border-bottom:2px solid ${accent}">
          ...
          <div style="text-align:right">
            ...
            ${profile.email?`<div style="font-size:13px;color:#555">✉ ${profile.email}</div>`:''}
            ${window.ResumeHelpers.contactDivs(profile, 'font-size:13px;color:#555;margin-bottom:3px;word-break:break-all')}
          </div>
        </div>

        ${profile.summary?`${sHead('Objective / Summary')}<div style="font-size:13px;color:#444;line-height:1.6">${profile.summary}</div>`:''}
        ...
        ${personalInfo.length||hobbies.length?`
          <div style="display:grid;grid-template-columns:${personalInfo.length&&hobbies.length?'1fr 1fr':'1fr'};gap:12px;margin-top:4px">
            ${personalInfo.length?`<div>${sHead('Personal Information')}${personalInfo.map(([k,v])=>`<div style="font-size:12px;color:#555;margin-bottom:2px"><span style="color:#999">${k}: </span>${v}</div>`).join('')}</div>`:''}
            ${hobbies.length?`<div>${sHead('Hobbies')}<div style="display:flex;flex-wrap:wrap;gap:3px">${hobbies.map(h=>`<span style="background:#f0fdf4;border:1px solid #bbf7d0;color:#15803d;padding:1px 7px;border-radius:10px;font-size:12px">${h}</span>`).join('')}</div></div>`:''}
          </div>
        `:''}

        ${personal.decl?`
          ${sHead('Declaration')}
          <div style="font-size:12px;color:#555;line-height:1.6">${personal.decl}</div>
