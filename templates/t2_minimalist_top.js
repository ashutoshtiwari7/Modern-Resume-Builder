  ...
  render(data, accentColor) {
    ...
    const sectionHead = (title) => `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px;margin-top:14px">
        ...
        <span style="font-size:9.5px;font-weight:700;color:#1a1a1a;text-transform:uppercase;letter-spacing:1.5px">${title}</span>
    ...
    const workHTML = work.length ? work.map(w => `
      <div style="margin-bottom:9px;padding-left:11px;border-left:2px solid #f0f0f0">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            <div style="font-weight:700;font-size:9.5px;color:#1a1a1a">${w.role || ''}</div>
            <div style="font-size:8.5px;color:${accent};font-weight:600">${w.company || ''}</div>
          </div>
          <div style="font-size:7.5px;color:#888;white-space:nowrap;margin-left:8px">${[w.from,w.to].filter(Boolean).join(' – ')}</div>
        </div>
        ${w.desc ? `<div style="font-size:8px;color:#555;line-height:1.6;margin-top:3px">${w.desc.replace(/\n/g,'<br>')}</div>` : ''}
      </div>`).join('') : '';

    const eduHTML = edu.length ? edu.map(e => `
      <div style="margin-bottom:8px;display:flex;justify-content:space-between">
        <div>
          <div style="font-weight:700;font-size:9px;color:#1a1a1a">${e.degree || ''}</div>
          <div style="font-size:8.5px;color:${accent}">${e.school || ''}</div>
          ${e.gpa ? `<div style="font-size:7.5px;color:#888">GPA: ${e.gpa}</div>` : ''}
        </div>
        <div style="font-size:7.5px;color:#888;white-space:nowrap;margin-left:8px">${[e.from,e.to].filter(Boolean).join(' – ')}</div>
      </div>`).join('') : '';

    const skillsHTML = skills.length ? `
      <div style="display:flex;flex-wrap:wrap;gap:5px">
        ${skills.map(s=>`<span style="border:1.5px solid ${accent};color:${accent};padding:2px 9px;border-radius:3px;font-size:7.5px;font-weight:600">${s.name}</span>`).join('')}
      </div>` : '';

    const hobbiesHTML = hobbies.length ? `<div style="font-size:8px;color:#555">${hobbies.join(' · ')}</div>` : '';
    ...
    const customHTML = (customSections||[]).map(sec => `
      ...
      ${sec.items.map(it=>`<div style="margin-bottom:7px;padding-left:11px;border-left:2px solid #f0f0f0"><div style="font-weight:700;font-size:9px;color:#1a1a1a">${it.title||''}</div>${it.subtitle?`<div style="font-size:8px;color:${accent}">${it.subtitle}</div>`:''}<div style="font-size:7.5px;color:#888">${it.date||''}</div>${it.desc?`<div style="font-size:8px;color:#555;line-height:1.5;margin-top:2px">${it.desc.replace(/\n/g,'<br>')}</div>`:''}</div>`).join('')}
    `).join('');
    ...
    return `
      <div style="width:210mm;min-height:297mm;font-family:inherit;background:#fff;box-sizing:border-box">
        ...
        <div style="background:#1a1a1a;padding:24px 28px;display:flex;align-items:center;gap:18px">
          ...
          <div style="flex:1">
            <div style="font-size:22px;font-weight:700;color:#fff;letter-spacing:-0.5px;line-height:1.1">${profile.firstName||''} ${profile.lastName||''}</div>
            ${profile.tagline ? `<div style="font-size:9px;color:${accent};text-transform:uppercase;letter-spacing:2px;margin-top:4px">${profile.tagline}</div>` : ''}
            <div style="display:flex;gap:14px;margin-top:8px;flex-wrap:wrap">
              ${profile.phone?`<span style="font-size:8px;color:#aaa">📞 ${profile.phone}</span>`:''}
              ${profile.email?`<span style="font-size:8px;color:#aaa">✉ ${profile.email}</span>`:''}
              ${profile.website?`<span style="font-size:8px;color:#aaa">🔗 ${profile.website}</span>`:''}
            </div>
          </div>
        </div>
        ...
        <div style="padding:18px 28px;box-sizing:border-box">
          ${profile.summary ? `
            ${sectionHead('Profile')}
            <div style="font-size:8.5px;color:#444;line-height:1.7;padding-left:11px">${profile.summary}</div>
          ` : ''}
          ...
          ${personalRows.length ? `
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px 16px;padding-left:11px">
              ${personalRows.map(([k,v])=>`<div style="font-size:8px;color:#555"><span style="color:#999;margin-right:4px">${k}:</span>${v}</div>`).join('')}
            </div>
          ` : ''}
          ...
          ${personal.decl ? `
            ...
            <div style="font-size:8px;color:#666;padding-left:11px;line-height:1.6">${personal.decl}</div>
            <div style="font-size:7.5px;color:#aaa;padding-left:11px;margin-top:5px">Date: ___________ &nbsp;&nbsp; Place: ___________</div>
          ` : ''}
        </div>
      </div>`;
  }
