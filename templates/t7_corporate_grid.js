...
window.ResumeTemplates.t7_corporate_grid = {
  ...
  render(data, accentColor) {
    ...
    const photoHTML = window.ResumeHelpers.photoContent(profile, initials, `font-size:35px;font-weight:700;color:${accent}`);

    const sHeadMain = (title) => `
      <div style="margin:14px 0 8px">
        <div style="font-size:14px;font-weight:800;color:${accent};text-transform:uppercase;letter-spacing:2px">${title}</div>
    ...
    const sHeadSide = (title) => `
      <div style="margin:12px 0 7px">
        <div style="font-size:13px;font-weight:700;color:${accent};text-transform:uppercase;letter-spacing:1.5px">${title}</div>
    ...
    const workHTML = work.map(w => `
      <div class="resume-block" style="margin-bottom:10px">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div>
            <div style="font-weight:700;font-size:14px;color:#1f2937">${w.role||''}</div>
            <div style="font-size:13px;color:${accent};font-weight:600">${w.company||''}</div>
          </div>
          <div style="font-size:12px;color:#6b7280;background:#f3f4f6;padding:2px 6px;border-radius:3px;white-space:nowrap;margin-left:6px">${[w.from,w.to].filter(Boolean).join(' – ')}</div>
        </div>
        ${w.desc?`<div style="font-size:13px;color:#4b5563;line-height:1.6;margin-top:3px">${w.desc.replace(/\n/g,'<br>')}</div>`:''}
      </div>`).join('');

    const eduHTML = edu.map(e => `
      <div class="resume-block" style="margin-bottom:8px">
        <div style="font-weight:700;font-size:14px;color:#1f2937">${e.degree||''}</div>
        <div style="font-size:13px;color:${accent}">${e.school||''}</div>
        <div style="font-size:12px;color:#9ca3af">${[e.from,e.to].filter(Boolean).join(' – ')} ${e.gpa?'· '+e.gpa:''}</div>
      </div>`).join('');

    const skillsHTML = skills.map(s=>`
      <div class="resume-block" style="margin-bottom:6px">
        <div style="display:flex;justify-content:space-between;font-size:13px;color:#374151;margin-bottom:2px">
    ...
    const customHTML = (customSections||[]).map(sec => `
      ${sHeadMain(sec.title)}
      ${sec.items.map(it=>`<div class="resume-block" style="margin-bottom:8px"><div style="font-weight:700;font-size:14px;color:#1f2937">${it.title||''}</div>${it.subtitle?`<div style="font-size:13px;color:${accent}">${it.subtitle}</div>`:''}<div style="font-size:12px;color:#9ca3af">${it.date||''}</div>${it.desc?`<div style="font-size:13px;color:#4b5563;line-height:1.5">${it.desc.replace(/\n/g,'<br>')}</div>`:''}</div>`).join('')}
    `).join('');
      ...
      <div style="width:210mm;min-height:297mm;font-family:inherit;background:#f8f9fa;box-sizing:border-box">
        ...
        <div style="background:#fff;border-bottom:3px solid ${accent};padding:20px 24px;display:flex;align-items:center;gap:16px">
          ...
          <div style="text-align:right;display:flex;flex-direction:column;gap:4px">
            ...
            ${profile.email?`<div style="font-size:13px;color:#6b7280">✉ ${profile.email}</div>`:''}
            ${window.ResumeHelpers.contactDivs(profile, 'font-size:13px;color:#6b7280;margin-bottom:4px;word-break:break-all')}
          </div>
        </div>
        ...
        <div style="display:flex;gap:0">
          ...
          <div style="flex:0 0 130mm;padding:14px 18px 18px;box-sizing:border-box;background:#fff;border-right:1px solid #e5e7eb">
            ${profile.summary?`${sHeadMain('Professional Summary')}<div style="font-size:13px;color:#4b5563;line-height:1.7">${profile.summary}</div>`:''}
            ${work.length?`${sHeadMain('Work Experience')}${workHTML}`:''}
            ${edu.length?`${sHeadMain('Education')}${eduHTML}`:''}
            ${customHTML}
            ${personal.decl?`${sHeadMain('Declaration')}<div style="font-size:13px;color:#6b7280;line-height:1.6">${personal.decl}</div><div style="font-size:12px;color:#9ca3af;margin-top:4px">Date: ___________ &nbsp; Place: ___________</div>`:''}
          </div>
          <!-- RIGHT SIDEBAR (35%) -->
          <div style="flex:1;padding:14px 14px 18px;box-sizing:border-box">
            ${skillsHTML?`${sHeadSide('Skills')}${skillsHTML}`:''}
            ${personalInfo.length?`${sHeadSide('Personal Info')}${personalInfo.map(([k,v])=>`<div style="font-size:12px;color:#4b5563;margin-bottom:3px"><span style="color:#9ca3af">${k}: </span>${v}</div>`).join('')}`:''}
            ${hobbies.length?`${sHeadSide('Hobbies')}<div style="display:flex;flex-wrap:wrap;gap:3px">${hobbies.map(h=>`<span style="background:#ede9fe;color:${accent};padding:2px 7px;border-radius:10px;font-size:12px">${h}</span>`).join('')}</div>`:''}
          </div>
        </div>
      </div>`;
  }
